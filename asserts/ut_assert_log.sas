...
*/
    %ut_init(type=ut_assert_log, description=&description., expected_result=&expected_result.);
    ...
    *-- Define a macro variable to store the result of ut_search_log --*;
    ...
    %if %sysevalf(%superq(log_type) ne, boolean) %then  %let ut_det = Expected log type %nrbquote(&log_type.) with message:^n;
    %else                                               %let ut_det = Expected log message:^n;
    %let ut_det = &ut_det.%nrbquote(&log_msg.)^n;

    %if &ut_search_log. = TRUE %then %do;
        %let ut_res=PASS;
        %let ut_det = &ut_det.found in the SAS log;
    %end;
    %else %do;
        %let ut_res=FAIL;
        %let ut_det = &ut_det.not found in the SAS log;
    %end;
