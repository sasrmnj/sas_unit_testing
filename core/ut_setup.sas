...
*/
    ...
    *-- Define the working directory (used as a temporary directory to store the SAS logs generated by ut_run) --*;
    %if %sysevalf(%superq(work_dir) ne, boolean) %then %do;
        %if not %sysfunc(fileexist(%nrbquote(&work_dir.))) %then %do;
            %put ERROR: input parameter WORK_DIR does not exist.;
            %put ERROR- &work_dir.;
            %let syscc = 99;
            %return;
        %end;

        %let ut_work_dir = %nrbquote(&work_dir.);
    %end;
    %else %do;
        *-- Get the SAS work directory --*;
